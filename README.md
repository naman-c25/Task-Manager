...
### Backend → Render
- The included [`render.yaml`](render.yaml) provisions the web service, a managed
  **PostgreSQL** database, and a **Redis (Key Value)** instance, wiring
  `DATABASE_URL`/`REDIS_URL` automatically.
- **Root directory:** `backend`
- **Build command:** `npm install && npm run build && npm run prisma:deploy` (applies migrations)
- **Start command:** `npm start`
- Set `CLIENT_ORIGIN` to your deployed frontend URL. `JWT_ACCESS_SECRET` is generated by the blueprint.
